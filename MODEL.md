...
Language-conditioned action chunking transformer. Small imitation policy for LIBERO-Object (Franka in MuJoCo), 7.6M trainable parameters. Trains on the Object suite by default or on all 40 LIBERO tasks with `--all-tasks`. Not a 7B VLA. Not the x500.

| | |
| --- | --- |
| Name | LC-ACT |
| Package | `lc_act` |
| Primary eval | All 10 LIBERO-Object tasks; `lc_act.eval` runs task 0, “pick up the alphabet soup and place it in the basket” |
| Done bar | ≥2 / 10 soup episodes + a rollout video (met; current weights: 95% over all 10 tasks) |
| Recipe | ACT (Zhao et al., 2023) + OFT L1/chunks (Kim et al., 2025) + LIBERO ResNet-T language (Liu et al., 2023) + MINERVA small-policy findings (Sendai et al., 2026) |
...
Scratch Diffusion Policy already hits **92.5% Object**. MINERVA reaches 99.6% Object with a 0.54M-parameter policy at 128 px, where L1 regression matched flow matching and replan-every-step temporal ensembling beat long open-loop chunks. We need spatial chunks + L1, not a giant VLM.
...
The diagram matches the code, including token tagging.
...
With `--n-obs 2`, each sample carries the previous frame and state (oldest first). Both frames go through the shared ResNet with one shared random shift, and each token concatenates the two frames' 256-d features before the linear projection, so the token count stays the same. The state token projects the 2×8 stacked states.
...
- Dataset: [`lerobot/libero`](https://huggingface.co/datasets/lerobot/libero) (~1.9 GB). LeRobot `video_backend="pyav"`; the installable package is `av`. By default keep `pick up the .+ and place it in the basket` (454 episodes, 67k frames); `--all-tasks` keeps all 40 tasks (1,693 episodes, 273k frames).
- History (`--n-obs 2`): LeRobot delta timestamps fetch the previous row; at episode start it repeats the first frame.
- Normalize state/action with the training subset's mean/std; store stats in the checkpoint.
- Eval images: flip H and W (dataset is 180° from raw MuJoCo).
- Env: single gym `LiberoEnv`, `libero_object`, relative 7-D, 280 steps. `lc_act.eval` runs task 0; the 10-task numbers use the same `run_episode` over task ids 0–9.
...
- Keep H = 16: MINERVA found chunk 8 and chunk 32 each cost 2–3 points on LIBERO.
...
- Sendai et al., MINERVA: How Small Can a Manipulation Policy Be and Still Solve LIBERO?, arXiv:2609.03715, 2026.
